...
class BFTIntegrations {
	...
	static function contact_form() {
		...
		if(!empty($_POST['classes']) and check_admin_referer('arigato_integrate')) {
			$shortcode_atts .= ' css_classes="'.sanitize_text_field($_POST['classes']).'" ';
		}
		if(!empty($_POST['html_id']) and check_admin_referer('arigato_integrate')) {
			$shortcode_atts .= ' html_id="'.sanitize_text_field($_POST['html_id']).'" ';
		}
		
		// change your-name and your-email to custom field names
		if(!empty($_POST['change_defaults']) and check_admin_referer('arigato_integrate')) {
	...
	static function wpforms() {
		...
		if(!defined('WPFORMS_VERSION')) {
			wp_die(__('WPForms is not activated.', 'broadfast'));
		} 

		
		if(!empty($_POST['ok']) and check_admin_referer('bft_wpforms')) {
			// save integration
			$integration = array("form_id" => intval($_POST['form_id']), "fields" => [] );
			$integration['fields']['email'] = sanitize_text_field($_POST['field_email']);
			$integration['fields']['name'] = sanitize_text_field($_POST['field_name']);
			...
			update_option('bft_wpforms_integration', $integration);
		} 
			
		// unserialize current integration
		$integration = get_option('bft_wpforms_integration');
		...
		// select wpforms forms
		$forms = $wpdb->get_results("SELECT ID, post_title FROM {$wpdb->posts} WHERE post_type='wpforms' ORDER BY post_title");
		$forms = wp_unslash($forms);
		
		// form selected? get fields
		$wpforms_fields = [];		
		if($selected_form_id) {
			$wpform_fields = $wpdb->get_var($wpdb->prepare("SELECT post_content FROM {$wpdb->posts} WHERE ID=%d", $selected_form_id));
			$wpform_fields = json_decode($wpform_fields);
			$wpform_fields = $wpform_fields->fields;			
		}
		
		require(BFT_PATH."/views/integration-wpforms.html.php");
	} // end WPForms settings
	
	// actually handle the formidable signup
	static function signup_wpforms($fields, $entry, $form_data, $entry_id) {
		global  $wpdb;	
		
		$wpform_fields = $wpdb->get_var($wpdb->prepare("SELECT post_content FROM {$wpdb->posts} WHERE ID=%d", $form_data['id']));
        $wpform_fields = json_decode($wpform_fields);
		$wpform_fields = $wpform_fields->fields;			
		...
		if(empty($integration['form_id']) or $integration['form_id'] != $form_data['id']) return false;

		// is checkbox required?
        if(!empty($integration['fields']['checkbox'])) {
            $integrate = false;
            foreach($wpform_fields as $field) {
                if($field->id == $integration['fields']['checkbox'] and !empty($entry_fields[$field->id][0] ?? null)) $integrate = true;
            }
            if(!$integrate) return false;			
        } // end checkbox check
		...
		foreach($integration['fields'] as $key => $field_id) {				
            ...
            foreach($wpform_fields as $field) {
                    if($field->id == $field_id) {						
                        if(!empty($entry_fields[$field->id])) {
                            $email = sanitize_email($entry_fields[$field->id]);
                        }
                        break;						 	
                    } // end if field found
                } // end foreach ninja field
            ...
            if($key == 'name') {
                foreach($wpform_fields as $field) {
                    if($field->id == $field_id) {						
                        ...
                        if(!empty($entry_fields[$field->id])) {
                            //echo "NONEMPTY";
                           $name = sanitize_text_field(implode(' ',$entry_fields[$field->id]));
                        }
                        break;						 	
                    } // end if field found
                } // end foreach ninja field
