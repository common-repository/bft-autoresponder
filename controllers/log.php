...
function bft_log() {
	...
	if(!empty($_POST['cleanup']) and check_admin_referer('bft_log')) update_option('bft_cleanup_raw_log', intval($_POST['cleanup_days']));
	
	$cleanup_raw_log = get_option('bft_cleanup_raw_log');
	if(empty($cleanup_raw_log)) $cleanup_raw_log = 7;
	$cleanup_raw_log = intval($cleanup_raw_log);
